...
function playBeep(scene, freq = 440, duration = 100){
...
const particles = this.add.particles(0, 0, null, {
  ...
  const more = this.add.text(0,50,"More Content Soon",{fontSize:"16px",color:"#888"})
    .setOrigin(0.5);
  ...
  playBtn.on("pointerdown", ()=>{
    playBeep(this,700,120);
    this.cameras.main.fadeOut(400);
    this.time.delayedCall(400,()=>this.scene.start("SoloScene"));
  });

  partyBtn.on("pointerdown", ()=>{
    playBeep(this,500,120);
    this.cameras.main.fadeOut(400);
    this.time.delayedCall(400,()=>this.scene.start("PartyScene"));
  });
};
